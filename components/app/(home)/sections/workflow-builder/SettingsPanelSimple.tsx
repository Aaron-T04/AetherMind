...
export default function SettingsPanel({ isOpen, onClose }: SettingsPanelProps) {
  ...
  // HACKATHON DEMO: Use demo user ID if no user is logged in
  const demoUserId = "demo-user-hackathon-2025";
  ...
  const [selectedProvider, setSelectedProvider] = useState<'anthropic' | 'openai' | 'groq' | 'gemini' | 'aimlapi' | null>(null);
              ...
              <div className="space-y-20">
                ...
                <div>
                  ...
                  <div className="space-y-8">
                    {['anthropic', 'openai', 'groq', 'gemini', 'aimlapi'].map(provider => {
                      ...
                      const hasEnvKey = provider === 'anthropic' ? serverConfig?.anthropicConfigured :
                                       ...
                                       provider === 'gemini' ? serverConfig?.geminiConfigured :
                                       serverConfig?.aimlapiConfigured;
                              ...
                              {providerKey && (
                                <>
                                  <button
                                    onClick={async () => {
                                      if (effectiveUserId) {
                                        await deleteLLMKey({ id: providerKey._id, userId: effectiveUserId });
                  ...
                  <div className="grid grid-cols-1 gap-8">
                    <div className="flex items-center gap-8 p-10 bg-background-base rounded-8 border border-border-faint">
                      ...
                      <div className="min-w-0">
                        <p className="text-body-small text-accent-black font-medium truncate">Web Research</p>
                      </div>
                    </div>
                  </div>
                </div>
            ...
            {server.tools && server.tools.length > 0 && (
              <div>
                ...
                <div className="flex flex-wrap gap-4">
                  {server.tools
                    .filter((tool) => tool && tool.trim()) // Filter out empty/whitespace tools
                    .map((tool, index) => (
                      <span key={`${server._id}-tool-${index}-${tool}`} className="px-8 py-4 bg-black-alpha-4 text-xs text-accent-black rounded-4">
                        {tool}
                      </span>
                    ))}
                </div>
              ...
              <div className="p-12 bg-heat-4 rounded-8 border border-heat-100">
                <div className="flex flex-wrap gap-4">
                  {discoveredTools
                    .filter((tool) => tool && tool.trim()) // Filter out empty/whitespace tools
                    .map((tool, index) => (
                      <span key={`discovered-tool-${index}-${tool}`} className="px-6 py-2 bg-white text-heat-100 rounded-4 text-xs font-medium border border-heat-100">
                        {tool}
                      </span>
                    ))}
                </div>
...
interface AddLLMKeyModalProps {
  isOpen: boolean;
  onClose: () => void;
  selectedProvider: 'anthropic' | 'openai' | 'groq' | 'gemini' | 'aimlapi' | null;
        ...
        <div className="p-24 space-y-16 overflow-y-auto flex-1">
          <div>
            ...
            <select
              ...
              onChange={(e) => setFormData({ ...formData, provider: e.target.value as 'anthropic' | 'openai' | 'groq' | 'gemini' | 'aimlapi' })}
            ...
            <div className="relative">
              <input
                ...
                placeholder={
                  ...
                  formData.provider === 'aimlapi' ? 'your-aimlapi-key' :
                  'Enter your API key'
                }
